...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    // Listen to push notifications arriving while the user is actively on this page
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const formatted: CakeNotification[] = (data || []).map((n: any) => ({
        id: n.id,
        orderId: n.orders?.order_number || n.order_id.substring(0, 8),
        status: n.status,
        title: n.title,
        body: n.body,
        isRead: n.is_read,
      ...
      {loading ? (
        ...
        >
          {notifications.map((item) => {
            ...
            const itemName = item.orders?.products?.name || 'Item';
            return (
              <TouchableOpacity 
                key={item.id} 
...
// Keep your original styles object identical
